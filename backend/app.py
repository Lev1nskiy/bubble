...
def login():
    if request.method == "POST":
        if "psw" in request.form:
            ...
            # Регистрация
            if is_valid_name(request.form['name']) and is_valid_surname(
                    request.form['surname']) and is_valid_patronymic(request.form['patronymic']) and is_valid_login(
                    request.form['login']) and is_valid_email(request.form['email']) and is_valid_password(
                    request.form['psw1']) and request.form['psw1'] == request.form['psw2']:
                ...
                res = dbase.addUser(request.form['name'], request.form['surname'], request.form['patronymic'],
                                    request.form['login'], request.form['email'], hash, 'user')
                if res:
...
def add_user():
    if request.method == 'POST':
        if is_valid_name(request.form['name']) and is_valid_surname(request.form['surname']) and is_valid_patronymic(
                request.form['patronymic']) and is_valid_login(request.form['login']) and is_valid_email(
                request.form['email']) and is_valid_password(request.form['psw1']) and request.form['psw1'] == \
                request.form['psw2']:
            hash = generate_password_hash(request.form['psw1'])
            res = dbase.addUser(request.form['name'], request.form['surname'], request.form['patronymic'],
                                request.form['login'], request.form['email'], hash, request.form['role'])
            if res:
                flash('Пользователь успешно зарегистрирован', "success")
                return redirect(url_for('profile'))
            else:
